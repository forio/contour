...
describe('utils', function () {
    describe('dateDiff', function () {
        it('should return the difference in days of two dates', function () {
            ...
            expect(nwt.dateDiff(d2, d1)).toBe(1);
        });
    });

    describe('roundToNearest', function () {
        it('should round the given number to the neares specified multiple', function () {
            expect(nwt.roundToNearest(7, 14)).toBe(14);
            expect(nwt.roundToNearest(7, 10)).toBe(10);
        });
    });
    ...
    describe('maxTickValues', function () {
        it('should return domain if domain length is less then maxTicks', function () {
            var domain = [1,2,3];
            expect(nwt.maxTickValues(5, domain)).toEqual([1,2,3]);
        });

        it('should return domain if domain is empty', function () {
            var domain = [];
            expect(nwt.maxTickValues(5, domain)).toEqual([]);
        });

        it('should return domain if domain length is equal to maxTicks', function () {
            var domain = [1,2,3];
            expect(nwt.maxTickValues(3, domain)).toEqual([1,2,3]);
        });
        ...
        describe('when maxTicks is less than domain length', function () {
            ...
            beforeEach(function () {
                ticks = nwt.maxTickValues(maxTicks, domain);
            });
            ...
            it('should return evenly spaced ticks', function () {
                ticks = nwt.maxTickValues(5, domain);
    ...
    describe('normalizeSeries', function () {
        ...
        it('should sort data if its not categorized ', function () {
            ...
            var series = nwt.normalizeSeries(data);
            expect(series).toBeSorted();
        });

        it('should not sort data if its categorized', function () {
            var data = [ 5,6,7];
            var series = nwt.normalizeSeries(data, ['d', 'x', 'a']);
        ...
        describe('when passing a categories array', function () {
            it('should use the categories array for normalized x values', function () {
                ...
                var series = nwt.normalizeSeries(data, cats);
                var s1 = series[0];
    ...
    describe('isDate', function () {
        it('should return true for date objects', function () {
            expect(nwt.isDate(new Date())).toBe(true);
        });

        it('should return false for other objects', function () {
            expect(nwt.isDate({})).toBe(false);
        });
    });

    describe('isNumber', function () {
        it('should return true for date objects', function () {
            expect(nwt.isNumber(5)).toBe(true);
        });

        it('should return false for other objects', function () {
            expect(nwt.isNumber('5')).toBe(false);
            expect(nwt.isNumber('hello')).toBe(false);
            expect(nwt.isNumber({})).toBe(false);
            expect(nwt.isNumber(new Date())).toBe(false);
            expect(nwt.isNumber(null)).toBe(false);
        });
    });

    describe('range', function () {
        it('should create range with default step 1', function () {
            expect(nwt.range(1,5)).toEqual([1,2,3,4])
        });

        it('if only one param it should be the size of the range [0...n]', function () {
            expect(nwt.range(5)).toEqual([0,1,2,3,4])
        });

        it('should accept a step', function () {
            expect(nwt.range(1, 5, 2)).toEqual([1,3])
        });

        it('should get emtpy range if 0', function () {
            expect(nwt.range(0)).toEqual([]);
        });

        it('should accept negative ranges', function () {
            expect(nwt.range(-4)).toEqual([0, -1, -2, -3]);
            expect(nwt.range(0, -4, -1)).toEqual([0, -1, -2, -3]);
        });

        it('should accept fractional ranges', function () {
            expect(nwt.range(0, 2, .5)).toEqual([0, .5, 1, 1.5]);
        })
    });

    describe('uniq', function () {
        it('should return empty array if null of not array', function () {
            expect(nwt.uniq(5)).toEqual([]);
            expect(nwt.uniq(null)).toEqual([]);
            expect(nwt.uniq([])).toEqual([]);
        });

        it('should return uniq values', function () {
            // expect(nwt.uniq([1,2,3,4])).toEqual([1,2,3,4]);
            expect(nwt.uniq([1,2,3,4,4])).toEqual([1,2,3,4]);
            expect(nwt.uniq([1,2,3,3,4])).toEqual([1,2,3,4]);
            expect(nwt.uniq([1,1, 2, 2 ,3, 3, 4, 4])).toEqual([1,2,3,4]);
        })
    });
    ...
    describe('compact', function () {
        it('should remove falsy elements', function () {
            expect(nwt.compact([0, 1, 2, undefined, null, NaN, '', 3, undefined])).toEqual([1,2,3]);
        })
    });
    ...
    describe('intersection', function () {
        it('should return the common elements between two arrays', function () {
            var a = [1,2,3,4,5];
            var b = [2,4,6,8,10,12];
            expect(nwt.intersection(a, b)).toEqual([2,4]);
        });
        ...
        it('should preseve the order of the first array', function () {
            ...
            var res = nwt.intersection(a, b);
            expect(res).toEqual([5,2,8]);
        });

        it('should return empty array if any array is null or empty', function () {
            expect(nwt.intersection(null, [1,2,3])).toEqual([]);
            expect(nwt.intersection([1,2,3], null)).toEqual([]);
            expect(nwt.intersection([1,2,3], [])).toEqual([]);
            expect(nwt.intersection([], [1,2,3])).toEqual([]);
        });
    });
    ...
    describe('merge', function () {
        it('should merge all props in both objects', function () {
            ...
            expect(nwt.merge(a, b)).toEqual({ a: 1, b: 2, c: 3, x: 4 });
        });
        ...
        it('should override common non mergable props with the second object', function () {
            ...
            expect(nwt.merge(a, b)).toEqual({ a: 3, b: 2, x: 4 });
        });

        it('should deep merge simple mergable object', function () {
            var a = { a: { x: 1 }, b: 2 };
            var b = { a: { y: 2 }, c: 3 };
            expect(nwt.merge(a, b)).toEqual({ a: { x: 1, y: 2 }, b: 2, c: 3 });
        });
        ...
        it('should skip undefined props from source object that exist in target', function () {
            var a = { a: 1, b: 2 };
            var b = { b: undefined };
            expect(nwt.merge(a, b)).toEqual({ a: 1, b: 2 });
        });
    ...
    describe('defaults', function () {
        it('should copy properties that resolve to undefined in target', function () {
            expect(nwt.defaults({a: 1}, {a: 3, b: 2})).toEqual({ a: 1, b: 2 });
        });

        it('should skip props that are already set by previous source', function () {
            expect(nwt.defaults({ a: 1 }, {a: 3, b: 2}, { b: 4 })).toEqual({ a: 1, b: 2 });
        });
    });
    ...
    describe('omit', function () {
        it('should return a copy of the original object', function () {
            var a = { x: 1, y: 2 };
            expect(nwt.omit(a, ['x'])).not.toBe(a);
        });

        it('should omit the props in the props array param', function () {
            var a = { x: 1, y: 2 };
            expect(nwt.omit(a, ['y'])).toEqual({ x: 1 });
        });
